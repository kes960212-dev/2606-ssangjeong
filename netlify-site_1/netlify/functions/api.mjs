...
const WRITE = new Set(['check', 'addItem', 'save', 'addEvent', 'addMemo', 'addSupply', 'setSupply', 'delRow', 'saveRecords']);
const QMAX = 300;
...
async function preview(st, a, p, pending) {
  try {
    ...
    if (a === 'check') {
      ...
      d.events = (d.events || []).concat([{ date: String(p.date || ''), text: String(p.text || ''), who, pending: true }])
        .sort((x, y) => (x.date < y.date ? -1 : x.date > y.date ? 1 : 0));
    } else if (a === 'addMemo') {
      d.memos = [{ row: 0, at: now, who, text: String(p.text || ''), color: String(p.color || ''), pending: true }].concat(d.memos || []);
    } else if (a === 'addSupply') {
      d.supplies = [{ row: 0, at: now, who, name: String(p.name || ''), qty: String(p.qty || ''), price: String(p.price || ''), use: String(p.use || ''), status: '요청', pending: true }].concat(d.supplies || []);
    } else if (a === 'setSupply') {
      const x = (d.supplies || []).find((s) => s.row === Number(p.row));
      if (x) { x.status = String(p.status || '요청'); x.pending = true; }
    } else if (a === 'delRow') {
      const key = { memo: 'memos', supply: 'supplies', event: 'events' }[String(p.tab)];
